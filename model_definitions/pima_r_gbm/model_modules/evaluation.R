...
evaluate <- function(data_conf, model_conf, ...) {
    ...
    png(paste(ifelse(model_conf[["outputPath"]] != "", model_conf[["outputPath"]], "artifacts/output/"), "confusion_matrix.png", sep=""), width = 860, height = 860)
    ...
    # Save metrics
    write(jsonlite::toJSON(metrics, auto_unbox = TRUE, null = "null", keep_vec_names=TRUE), paste(ifelse(model_conf[["outputPath"]] != "", model_conf[["outputPath"]], "artifacts/output/"), "metrics.json", sep=""))
}