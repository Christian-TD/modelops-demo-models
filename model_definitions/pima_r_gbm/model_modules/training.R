...
train <- function(data_conf, model_conf, ...) {
    ...
    print(paste(ifelse(model_conf[["outputPath"]] != "", model_conf[["outputPath"]], "artifacts/output/"), "model.rds", sep=""))
    #saveRDS(model, paste(ifelse(model_conf[["outputPath"]] != "", model_conf[["outputPath"]], "artifacts/output/"), "model.rds", sep=""))
